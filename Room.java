...
public class Room extends Application {
    private static TableView<RoomTimetable> table;
    ...
    public void start(Stage primaryStage) {
        primaryStage.setTitle("Room Timetable");

        BorderPane root = new BorderPane();
        Scene scene = new Scene(root, 800, 400);

        Label title = new Label("Room Timetable");
        title.setFont(new Font("Arial", 24));
        root.setTop(title);
        BorderPane.setMargin(title, new Insets(10, 10, 10, 10));

        table = new TableView<>();
        TableColumn<RoomTimetable, String> roomNumberCol = new TableColumn<>("Room Number");
        roomNumberCol.setCellValueFactory(cellData -> cellData.getValue().roomNumberProperty());

        TableColumn<RoomTimetable, String> courseNameCol = new TableColumn<>("Course Name");
        courseNameCol.setCellValueFactory(cellData -> cellData.getValue().courseNameProperty());

        TableColumn<RoomTimetable, String> classDayCol = new TableColumn<>("Class Day");
        classDayCol.setCellValueFactory(cellData -> cellData.getValue().classDayProperty());

        TableColumn<RoomTimetable, String> startTimeCol = new TableColumn<>("Start Time");
        startTimeCol.setCellValueFactory(cellData -> cellData.getValue().startTimeProperty());

        TableColumn<RoomTimetable, String> endTimeCol = new TableColumn<>("End Time");
        endTimeCol.setCellValueFactory(cellData -> cellData.getValue().endTimeProperty());

        table.getColumns().addAll(roomNumberCol, courseNameCol, classDayCol, startTimeCol, endTimeCol);
    ...
    public static void loadData() {
        ...
        try {
            ...
            String query = "SELECT * FROM RoomTimetables";
    ...
    private void searchTimetable() {
        ...
        if (searchText.isEmpty()) {
            ...
            alert.setContentText("Please enter a room number for search.");
            alert.showAndWait();
            return;
        }
        ...
        try {
            Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/hope", "root", "123blink!");
            String query = "SELECT * FROM RoomTimetables WHERE RoomNumber = ?";
